...
/**
    ...
    /**
     * Tell Flutter to disable/grayout teacher card
     */
    informFlutterDisable(teacherId) {
        if (window.flutter_inappwebview && window.flutter_inappwebview.callHandler) {
            window.flutter_inappwebview.callHandler('disableTeacher', teacherId);
            console.log('📱 Flutter notified: disable teacher', teacherId);
        }
    }

    /**
     * Tell Flutter teacher was evaluated (after success)
     */
    informFlutterEvaluated(teacherId) {
        if (window.flutter_inappwebview && window.flutter_inappwebview.callHandler) {
            window.flutter_inappwebview.callHandler('markTeacherEvaluated', teacherId);
            console.log('📱 Flutter notified: teacher evaluated', teacherId);
        }
    }
    ...
    setupClickInterception() {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (teacherInfo && teacherInfo.teacherId) {
                ...
                self.checkTeacherEvaluated(teacherInfo.teacherId, teacherInfo.teacherName).then(blocked => {
                    if (!blocked) e.target.click(); // Re-trigger if allowed
                });
                return false;
            }
        }, true); // CAPTURE phase
